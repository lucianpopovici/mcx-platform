...
### 4.40 CA-27 — voice codecs: numbers, layouts and the direction of a payload type

The codec layer (PLT-VP-R1 MED-OP-01) was written against these sources:

| Constant or rule | Source | Where |
|---|---|---|
| Static audio payload types 0 PCMU/8000, 8 PCMA/8000, 9 G722/8000; G.722's SDP clock rate 8000 | RFC 3551 table 4, 4.5.2 | `codec.STATIC_PAYLOAD_TYPES` |
| AMR/AMR-WB `octet-align`, `crc`, `robust-sorting`, `interleaving` are declarative and must be equal in offer and answer; defaults 0, and no interleaving | RFC 4867 8.3.1, 8.1 | `codec.MUST_MATCH` |
| AMR-WB mandatory for MCPTT clients, EVS optional; the preference order is operator policy; the bandwidth-efficient format shall be supported | TS 26.179 4.1.1, 4.1.2, 4.1.3 (V13.2.0 and V19.0.0 in docs/3GPP read the same) | `codec.choose` |
| A number in an SDP is what its author expects to receive; the offerer sends with the answer's numbers, the answerer with the offer's | RFC 3264 5.1, 6.1 | `Endpoint.rx_pt`, `tx_pt` |
| A dynamic number's mapping must not change within a session | RFC 3264 8.3.2 | `SipCore._relay_sdp` (the relay's offers keep its number) |
| The payload type is 7 bits | RFC 3550 5.1 | `codec.payload_type` |
| Encoding names compare without case | RFC 4855 3 | `codec.key_of` |

The first version had the direction of RFC 3264 backwards: it took the
number in a party's SDP as the one that party sends with. A callee that
answered the relay's 97 with 101 would then have had every talk burst
...
TS 26.179 was first read from the ETSI copy online and then checked against
docs/3GPP: clause 4 is the same in V13.2.0 and V19.0.0, and the citations
stand. Three further points were read there. 4.1.2: the codec preference order
is operator policy, which is what the profile's order is. 4.1.3: clients
"shall offer no particular mode set", so the unreconciled mode-set of
PLT-VP-R1 MED-OP-05 should not arise between conformant clients. 4.2.1: "MCPTT
call media codec information shall include codecs according to MCPTT Client
capabilities as specified in clause 4.1.1", that is, AMR-WB (and EVS). A call
whose codec is G.722 or G.711, chosen because the caller offered no AMR-WB or
EVS, offers an MCPTT client only a codec 4.1.1 does not require it to have.
That is the decided consequence of no transcoding (MED-OP-03), and such calls
are for interworking parties; recorded, not changed.
...
| CA-27 | Voice codec constants: static payload types, AMR/AMR-WB layout parameters, EVS parameters, payload type direction | RFC 3551, RFC 4867 8.3.1, RFC 3264 5.1/6.1/8.3.2, RFC 3550 5.1, TS 26.179 4.1-4.2 (V13.2.0, V19.0.0), TS 26.445 A.3 | **Closed.** See 4.40. The EVS parameters were read from TS 26.445 annex A after the fact; one (`cmr`) had been missed (MED-OP-04). |
...
| Version | Date | Change |
|---|---|---|
| 2.10 | 2026-09-26 | CA-27: TS 26.179 checked against docs/3GPP (V13.2.0, V19.0.0); citations stand; 4.2.1 recorded against G.711/G.722 calls to MCPTT clients. |
