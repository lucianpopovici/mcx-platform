...
| ADHOC-OP-04 | **Open — for the profile owner.** A listed call is bounded only by the call type's `max_participants`, and `rec-broadcast` declares none. A caller may list any number of entries, and each is resolved (and audited) before admission; 20,000 entries cost about 7 s under the SIP lock (review, before the linear parse fix; the resolution cost remains). Options: require `max_participants` on every ad hoc call type (the owner picks REC's value, knowing the limit also refuses criteria that find more members); or a deployment-wide list limit (a required setting with no default). | FRMCS REC, robustness |
