...
| MED-OP-03 | **R4.** Transcoding (decided 2026-09-25): until then a call only works when every party shares the codec the caller's offer and the profile's order choose; a party that does not is refused (488) or hung up. | R4 |
| MED-OP-04 | **CLOSED 2026-09-26.** The EVS parameters were read from TS 26.445 annex A, now in docs/3GPP (V12.17.0, V16.4.0 and V19.1.0 agree). `hf-only` and `evs-mode-switch` are confirmed: default 0 (A.3.1), and "when [it] is offered ... and the payload type is accepted, the answerer shall not modify or remove" it (A.3.3.1). The reading found a third of the same kind that had been missed: `cmr` (−1, 0, 1; default 0). It decides whether a CMR is carried in the payload (1: in every packet; −1: none in primary mode), and it is now must-match too. An answerer may add any of the three when it was not offered (A.3.3.1); the relay cannot reframe a payload, so that is refused as a mismatch. `dtx`, `br`, `bw` and the AMR-WB IO `mode-set` limit what is sent, not how, and fall under MED-OP-05. Tests pin all three, offered and not; mutation: every mutant killed. TS 26.179 is still read online only (ETSI V19.0.0). | closed |
| MED-OP-05 | **Open, low consequence.** Offer/answer limits of the relay. (1) Parameters that restrict what the SDP's author wants to receive (AMR `mode-set`; EVS `br`, `bw`, `dtx`/`dtx-recv`, `ch-aw-recv`) are not reconciled across parties: every party is given the caller's, and a callee's narrower mode-set is not passed back to the caller. (2) TS 26.179 4.1.3 has clients offer `ptime` 20 and `maxptime` 240; the relay's SDP carries neither. (3) The answer to the caller has one audio and one floor m-line, whatever the offer had, where RFC 3264 6 requires as many m-lines as the offer (pre-existing). (4) telephone-event is dropped from the answer, which is legal. | R2 |
