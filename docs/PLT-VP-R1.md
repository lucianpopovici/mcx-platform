...
| MED-OP-04 | **Open, low consequence.** The EVS payload format parameters treated as must-match (`hf-only`, `evs-mode-switch`, both defaulting to 0) are from recollection of TS 26.445 annex A, which is not in docs/3GPP; the safe side was taken (a mismatch is refused, never relayed). TS 26.179 was read as ETSI TS 126 179 V19.0.0 online and is not archived in docs/3GPP either. | VP1-MED-001 (EVS only) |
| MED-OP-05 | **Open, low consequence.** Offer/answer limits of the relay. (1) Parameters that restrict what the SDP's author wants to receive (AMR `mode-set`, EVS `br`, `bw`) are not reconciled across parties: every party is given the caller's, and a callee's narrower mode-set is not passed back to the caller. (2) TS 26.179 4.1.3 has clients offer `ptime` 20 and `maxptime` 240; the relay's SDP carries neither. (3) The answer to the caller has one audio and one floor m-line, whatever the offer had, where RFC 3264 6 requires as many m-lines as the offer (pre-existing). (4) telephone-event is dropped from the answer, which is legal. | R2 |
