...
| ADHOC-OP-03 | **Open — FRMCS criteria need it.** No location reaches the platform from SIP: the `application/vnd.3gpp.mcptt-location-info+xml` body (17.2.2.1.1 item 14a) is not read. Criteria naming a location-dependent identity, such as `rec-area` or `track-controller`, therefore match nobody on a SIP call, and REC by area cannot be placed from a native client yet. The resolver side is done and tested with a location supplied. | FRMCS REC |
