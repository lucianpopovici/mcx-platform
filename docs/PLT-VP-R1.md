...
| ADHOC-OP-04 | **CLOSED 2026-09-25 (decided: a deployment-wide cap, required, no default).** `MCX_ADHOC_LIST_MAX` states how many entries an ad hoc caller's participant list may hold, whatever the call type. It is the deployment's counterpart of the service configuration's `<max-no-participants>` (TS 24.379 17.4.2.2 step 6), so a longer list is refused with warning 189. The check comes before any entry is resolved, which is its point: resolution, and its audit records, happen before the caller is authorised (PLT-ICD-001 ICD-OP-09). A missing, zero, negative or non-integer value refuses startup. Criteria are not affected; they remain bounded by the call type's `max_participants`. The call type limit still applies alongside the cap. Before the decision: `rec-broadcast` declares no `max_participants`, so its list was unbounded (20,000 entries cost about 7 s under the SIP lock). | closed |
