...
| ADHOC-OP-03 | **CLOSED 2026-09-25 (decided: cells declared in the profile).** The core now reads the client's location report (`application/vnd.3gpp.mcptt-location-info+xml`, TS 24.379 annex F.3), taking the serving NCGI or ECGI. The profile's new `identity.cells` maps a cell to location attributes such as `track_section` (PLT-ICD-001 v0.10 §2.8), so criteria such as `rec-area` resolve from a native client's call. Encrypted or malformed reports never refuse a call; they leave it without a location. Schemas for the location body were extracted verbatim for Rel-17, 18 and 20, and the test reports validate against them. Originally: **Open — FRMCS criteria need it.** No location reached the platform from SIP. | closed |
| PRF-OP-03 | **Open — for the profile owner.** Cell identities are network data: they change with the radio plan and differ between networks. As you decided, they are declared in the profile (§2.8). The FRMCS profile declares none, only a commented example, so REC by area works only once a deployment fills `identity.cells`. Options: keep them in the profile, one profile per network; or move the map to deployment data (like the groups file) with the same validation. How an identity holder's own location (`bind`) is reported is unchanged: location still arrives at `bind` from outside SIP. | FRMCS REC |
