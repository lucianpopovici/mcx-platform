...
| ADHOC-OP-05 | **Open, conformance (found in the ICD-OP-09 review).** An unauthorised ad hoc caller is refused with warning 100. TS 24.379 17.4.2.2 asks for 185 when the user may not initiate ad hoc calls (step 4) and 186 when the service does not support them (step 5). For an unauthorised emergency ad hoc call (step 3B) it also asks for a 403 carrying `<adhoc-emergency-ind>false`. Needs a reason code for ad hoc authorisation mapped to 185, release-gated like 187 and 189, and the 3B body. The one-refusal property holds either way. | R1 exit |
