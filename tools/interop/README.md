...
| `pki.py` | Two throwaway CAs and one certificate per party. The SIP cores' certificates come from a CA of their own (`core-ca.crt`, `pathLenConstraint` 0), everyone else's from `ca.crt` (PLT-ICD-001 ICD-OP-10); `trust.crt` holds both for the test clients. Each user's certificate names its `sip:` URI, which is the only identity it may assert directly to the platform. The core under test is trusted by its `<core>.interop.test` DNS name and its issuer, both stated in the network profile the harness writes (`MCX_NETWORK_FILE`, §2.8). Neither interop core authenticates users, so these runs test the trust mechanism, not a secure deployment. |
