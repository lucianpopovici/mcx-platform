...
def make(directory: Path, parties: Iterable[str],
    ...
    ca_key, ca, ca_ski = _ca("mcx-interop-ca", now)
    # pathLenConstraint 0: it issues core certificates directly, nothing else.
    core_key, core_ca, core_ski = _ca("mcx-interop-core-ca", now, path_length=0)
    ...
    for cn in parties:
        issuer, issuer_key, issuer_ski = (
            (core_ca, core_key, core_ski) if cn in cores else (ca, ca_key, ca_ski))
        key = rsa.generate_private_key(public_exponent=65537, key_size=2048)
        cert = (x509.CertificateBuilder()
